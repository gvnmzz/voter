/* This program is Copyright © 2013 Charo I. Del Genio */
...
/* Generates and returns a graphical power-law distributed sequence
with the given number of nodes and power-law exponent gamma */
...
{
	int i, anyone, pivot, l_ind=0, s_ind=0, h_ind=0, *lower, *higher, *same;
	...
	anyone = (int) (floor ( ( ( (double) rand() ) / ( (double) RAND_MAX ) ) * (double) n ));
	pivot  = s[anyone];
	
	for (i=0;i<n;i++) {
		if (s[i]<pivot) lower[l_ind++]=s[i];
		else {
			if (s[i]>pivot) higher[h_ind++]=s[i];
			else same[s_ind++]=s[i];
		}
	}
	
	quicksort(lower,l_ind);
	quicksort(higher,h_ind);
	
	if (h_ind) for (i=0;i<h_ind;i++) s[i]=higher[i];
	if (s_ind) for (i=h_ind;i<s_ind+h_ind;i++) s[i]=same[i-h_ind];
	if (l_ind) for (i=s_ind+h_ind;i<h_ind+s_ind+l_ind;i++) s[i]=lower[i-h_ind-s_ind];
